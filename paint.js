...
p.View.Paint = p.View.Base.extend({
  ...
  render: function() {
    ...
    this.$container
      .find('.paint-color-button[data-color="#000000"]')
      .addClass("paint-active");
    this.$container.find(".paint-new").click(
      function(ev) {
        ...
        if (
          this.isBlank ||
          confirm("Achtung, nicht pfostierte Änderungen gehen verloren!")
        ) {
    ...
    this.filterView = new p.View.Filter(
      this.$container.find(".post-filter"),
      this
    );
  ...
  newDocument: function(width, height, type) {
    ...
    this.$container
      .find("div.paint-container")
      .toggleClass("animation", this.isAnimation);
  ...
  frameAdd: function() {
    ...
    var time = this.animationFrames.length
      ? this.animationFrames[this.animationFrameIndex].time
      : 250;
  ...
  frameSetCurrent: function(index) {
    ...
    var frame = this.animationFrames[this.animationFrameIndex];
    this.$container
      .find(".paint-animation-timeline canvas.paint-active")
      .removeClass("paint-active");
  ...
  frameSetAnimationTime: function(time) {
    this.animationTogglePreview(false);
    this.animationFrames[this.animationFrameIndex].time = Math.min(
      10000,
      Math.max(16, time)
    );
  },
  ...
  updateAnimationFrame: function() {
    if (this.isAnimation) {
      this.animationFrames[this.animationFrameIndex].ctx.drawImage(
        this.canvas,
        0,
        0
      );
  ...
  animationTogglePreview: function(force) {
    ...
    this.animationPreviewActive =
      force !== undefined ? force : !this.animationPreviewActive;
    ...
    this.$container
      .find(".paint-animation-toggle-preview")
      .toggleClass("paint-active", this.animationPreviewActive);
  },
  ...
  animationGenerateGIFNow: function(callback) {
    ...
    gif.on(
      ...
      function(blob) {
        ...
        )[0].src = this.animationObjectURL;
        var sizeMb = blob.size / (1024 * 1024);
        this.$container
          .find(".paint-animation-size")
          .text(Math.round(sizeMb * 1000) / 1000);
        if (callback) {
          callback();
        }
      }.bind(this)
  ...
  touchStart: function(ev) {
    ev.preventDefault();
    ev.stopPropagation();
    var p = this.getInternalPosition(
